 ...
 * @author Gabriel
 */
public class TelaListarCidade extends JFrame implements ActionListener {
    ...
    private JButton botaoCadastrar = new JButton("Nova Cidade", iconAdd);
    ...
    private JLabel labelCidade = new JLabel("Cidades:");
    ...
    public TelaListarCidade(Funcionario funcionario, Boolean tela) {
        ...
        addComponentePainelTop(botaoEstado, 3, 10, 10);
    ...
    private void atualizarTabela() {
        try {
            listaCidades = CidadeUC.buscarTodos();

            String[] titulos = {"ID", "Nome", "Estado"};
            Object[][] dados = new Object[listaCidades.size()][titulos.length];

            for (int i = 0; i < listaCidades.size(); i++) {
                dados[i][0] = listaCidades.get(i).getId();
                dados[i][1] = listaCidades.get(i).getNome();
                dados[i][2] = listaCidades.get(i).getEstado().getNome();
            }
    ...
    public void actionPerformed(ActionEvent evento) {
        ...
        if (evento.getSource().equals(botaoEditar)) {
            if (tabela.getSelectedRow() == -1) {
                ...
                new TelaCadastrarCidade(listaCidades.get(tabela.getSelectedRow())).setVisible(true);
            }
        ...
        if (evento.getSource().equals(botaoDeletar)) {
            if (tabela.getSelectedRow() == -1) {
                ...
                try {
                    CidadeUC.removerCidade(listaCidades.get(tabela.getSelectedRow()));
                } catch (Exception excecao) {
